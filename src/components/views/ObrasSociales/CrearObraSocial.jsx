...
import { crearObraSocial } from "../../helpers/queries";

const CrearObraSocial = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = (obraSocialNueva) => {
    console.log(obraSocialNueva);
    crearObraSocial(obraSocialNueva).then((respuesta) => {
      if (respuesta.status === 201) {
        Swal.fire(
          "Obra Social creada",
          `La Obra Social ${obraSocialNueva.nombreObraSocial} fue creada correctamente`,
          "success"
        );
        reset();
      } else {
        Swal.fire(
          "Ocurrio un error",
          `La Obra Social ${obraSocialNueva.nombreObraSocial} no pudo ser creada`,
          "error"
        );
      }
    });
...
export default CrearObraSocial;
